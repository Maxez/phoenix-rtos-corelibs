...
/* Returns host framebuffer resource format (RGBX big endian, XBGR little endian) */
static inline int virtiogpu_rgbx(void)
{
...
/* Returns host cursor icon resource format (XRGB big endian, BGRX little endian) */
...
int virtiogpu_mode(graph_t *graph, graph_mode_t mode, graph_freq_t freq)
{
	...
	if ((err = virtiogpu_create(vgpu, vgpu->req, virtiogpu_rgbx(), modes[i].width, modes[i].height, &res)) < 0)
		return err;
	...
	for (; info[virtiogpu_common.desc].type != vdevNONE; virtiogpu_common.desc++, virtiogpu_common.vctx.reset = 1) {
		while ((err = virtio_find(&info[virtiogpu_common.desc], &vdev, &virtiogpu_common.vctx)) != -ENODEV) {
			...
			do {
				...
				/* Create framebuffer */
				if ((err = virtiogpu_create(vgpu, vgpu->req, virtiogpu_rgbx(), vinfo.pmodes[0].r.w, vinfo.pmodes[0].r.h, &vgpu->fb)) < 0) {
					virtiogpu_put(vgpu->req);
					break;
				}
